...
/* ------------------------------------------------------------------ */
/*  Redlined PDF generation                                             */
/* ------------------------------------------------------------------ */

async function downloadRedlinedPDF(
  ndaText: string,
  clauses: FlaggedClause[],
  summary: string,
  ...
  doc.rect(margin - 2, y - 2, maxW + 4, 14, "F");
  doc.setDrawColor(217, 172, 95);
  doc.rect(margin - 2, y - 2, maxW + 4, 14, "S");
  doc.setFontSize(9);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(180, 120, 30);
  doc.text("DISCLAIMER: THIS IS NOT LEGAL ADVICE", margin + 2, y + 4);
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7);
  doc.setTextColor(120, 90, 30);
  doc.text(
    "Generated by AI agents. For informational purposes only. Consult a qualified lawyer before acting on findings.",
    margin + 2,
    y + 9
  );
  y += 18;
  ...
  doc.text("Jessica — NDA Risk Analysis", margin, y);
  y += 10;
  ...
  doc.text(`${counts.red} Red Flags`, margin, y);
  doc.setTextColor(200, 160, 60);
  doc.text(`${counts.yellow} Yellow Flags`, margin + 35, y);
  doc.setTextColor(80, 150, 80);
  doc.text(`${counts.green} Green Flags`, margin + 75, y);
  doc.setTextColor(120, 120, 130);
  doc.text(
    `${counts.red + counts.yellow + counts.green} total changes`,
    margin + 115,
    y
  );
  y += 8;
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(30, 30, 35);
  doc.text("General Counsel Summary", margin, y);
  y += 6;
  doc.setFontSize(8.5);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(60, 60, 65);
  const summaryLines = doc.splitTextToSize(summary || "No summary.", maxW);
  ...
  // --- FLAGGED CLAUSES (REDLINED) ---
  doc.setFontSize(11);
  doc.setFont("helvetica", "bold");
  doc.setTextColor(30, 30, 35);
  doc.text("Redlined Clauses", margin, y);
  y += 7;

  for (let i = 0; i < clauses.length; i++) {
    const c = clauses[i];
    const color = riskColors[c.risk_level] || riskColors.yellow;
    checkPage(35);

    // Clause header with risk badge
    doc.setFillColor(color[0], color[1], color[2]);
    doc.roundedRect(margin, y - 3, maxW, 7, 1, 1, "F");
    doc.setFontSize(9);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(255, 255, 255);
    doc.text(
      `${i + 1}. ${c.clause_type.toUpperCase()} — ${c.risk_level.toUpperCase()} RISK`,
      margin + 3,
      y + 1.5
    );
    y += 8;

    // Original text with left border (redline style)
    doc.setDrawColor(color[0], color[1], color[2]);
    doc.setLineWidth(0.8);
    const clauseLines = doc.splitTextToSize(
      c.original_text.slice(0, 400) + (c.original_text.length > 400 ? "..." : ""),
      maxW - 8
    );
    const blockH = clauseLines.length * 3.8 + 2;
    checkPage(blockH + 20);
    ...
    doc.rect(margin + 3, y - 1, maxW - 6, blockH, "F");
    doc.line(margin + 3, y - 1, margin + 3, y - 1 + blockH);
    ...
    doc.text("Rationale:", margin + 3, y);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(60, 60, 65);
    const rationaleLines = doc.splitTextToSize(c.explanation, maxW - 8);
    for (const line of rationaleLines.slice(0, 3)) {
      y += 3.5;
      checkPage(5);
    ...
    if (c.citation) {
      ...
      const citLines = doc.splitTextToSize(`Citation: ${c.citation}`, maxW - 8);
      for (const line of citLines.slice(0, 2)) {
        doc.text(line, margin + 3, y);
        y += 3.2;
      }
    }
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text(
      "Generated by Jessica AI Legal Team. This is NOT legal advice. Verify all citations independently.",
      margin,
  ...
  doc.save("jessica-nda-redlined-analysis.pdf");
}
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="flex items-center gap-3 rounded-lg border border-[rgb(217_172_95)]/40 bg-[rgb(217_172_95)]/[0.07] px-5 py-3.5"
      >
        <Scale className="size-5 text-[#d9ac5f] shrink-0" />
        <div>
          <span className="text-[11px] uppercase tracking-[0.2em] font-medium text-[#d9ac5f]">
            Disclaimer
          </span>
          <p className="text-xs text-[rgb(200_200_210)] leading-relaxed mt-0.5">
            This analysis is generated by AI agents and is <strong className="text-[#d9ac5f]">not legal advice</strong>. 
            It is intended for informational purposes only. Consult a qualified lawyer before acting on any findings. 
            AI output may contain errors — verify all citations independently.
          </p>
        </div>
      </motion.div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <button
          onClick={() => downloadRedlinedPDF(ndaText, clauses, gcSummary, counts)}
          disabled={clauses.length === 0}
          className="inline-flex items-center gap-2 px-5 py-2.5 text-[11px] uppercase tracking-[0.2em] font-medium border border-[rgb(217_172_95)] text-[#d9ac5f] hover:bg-[rgb(217_172_95)]/10 disabled:opacity-30 disabled:cursor-not-allowed transition-all duration-200 rounded-sm"
        >
